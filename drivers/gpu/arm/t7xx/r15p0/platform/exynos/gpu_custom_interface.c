...
static void sanitize_min_max(int *val, int min, int max) {
...
static int read_into(int *t, int size, const char *buf, size_t count)
{
	int i = 0;
	const char *p = buf;
	char *next;

	while (i < size && p < buf + count) {
		t[i] = simple_strtoul(p, &next, 10);
		if (p == next)
			break;
		p = next;
		while (p < buf + count && (*p == ' ' || *p == '\n' || *p == '\t'))
			p++;
		i++;
	}
	return i;
...
static ssize_t set_volt_table(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((tokens = read_into((int*)&t, min-max+1, buf, count)) < 0)
		return -EINVAL;
	...
	if (tokens == 2 && target > -1) {
		...
		for (i = 0; i < tokens; i++) {
            int target_idx = i + max;
            if (target_idx <= min) {
			    if ((rest = t[i] % GPU_VOLT_STEP) != 0) t[i] += GPU_VOLT_STEP-rest;
			    sanitize_min_max(&t[i], GPU_MIN_VOLT, GPU_MAX_VOLT);
			    platform->table[target_idx].voltage = t[i];
            }
		}
	}
